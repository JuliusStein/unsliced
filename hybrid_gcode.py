...
def makeSpiral(x, y, z_start, z_end, r, e_base, eject_rate, height, move_speed):
    ...
    lines = []
    lines.append("G1 X{} Y{} Z{} F{}\n".format(round(x+3, 2), round(y+3, 2), round(z_start+100, 2), 2400))
    lines.append("G1 X{} Y{} Z{} E{} F{}\n".format(round(x, 2), round(y, 2), round(z_start+3, 2), e_base, 2400))
    lines.append("G4 P500\n")
    
    for i, z_val in enumerate(z_values):
        t = 0.1*i*pi
        if i==0:
            lines.append("G1 X{} Y{} Z{} E{} F{}\n".format(x + round(r*cos(t), 2), y+ round(r*sin(t), 2), z_val, round(e_base, 2), move_speed))
        elif t<2*pi:
            lines.append("G1 X{} Y{} E{}\n".format(x + round(r*cos(t), 2), y+ round(r*sin(t), 2), round(e_base + eject_rate*i,2)))
        else:
            lines.append("G1 X{} Y{} Z{} E{}\n".format(x + round(r*cos(t), 2), y+ round(r*sin(t), 2), round(z_val-2*pi,2), round(e_base + eject_rate*i,2)))
...
e_start += 5
newLines += makeCollumn(0, -8.2, 3.4, 28.65, e_start, eject_rate, injectLayers, speed)
newLines += makeCollumn(-8.2, 0, 3.4, 28.65, e_start + (0.25*e_range), eject_rate, injectLayers, speed)
newLines += makeCollumn(0, 8.2, 3.3, 28.65, e_start + (0.5*e_range), eject_rate, injectLayers, speed)
newLines += makeCollumn(8.2, 0, 3.3, 28.65, e_start + (0.75*e_range), eject_rate, injectLayers, speed)

newLines.append(f"G92 E{e_stop}\n")
newLines += lines[layer_lines[endInject]:]

# write new gcode file
with open("TestObjectA_hybrid_v2.g", "w") as f:
